...
class Karnough:
    ...
    def check_figure(self, res, vertical_value, index, g_index, existing) -> bool:
        if index == 4:
            return False
        if vertical_value != self.CONSTANT_ONE:
            return False
        if res[g_index][index + 1] != self.CONSTANT_ONE:
            return False
        if g_index == 2 or res[g_index + 1][index] != self.CONSTANT_ONE:
            return False
        if res[g_index + 1][index + 1] != self.CONSTANT_ONE:
            return False
        cells = [[g_index, index], [g_index + 1, index + 1], [g_index + 1, index], [g_index, index + 1]]
        if self.checking_items(cells, existing):
            return False
        return True
    ...
    def figure_side(self, res, vertical, index, g_index, existing) -> bool:
        if g_index == 2:
            return False
        if index != 1 or vertical != self.CONSTANT_ONE:
            return False
        if res[g_index + 1][index] != self.CONSTANT_ONE:
            return False
        if res[g_index][4] != self.CONSTANT_ONE or res[g_index + 1][4] != self.CONSTANT_ONE:
            return False
    ...
    def karno_figure(values, verticals, gorizontals, existing):
        ...
        existing.extend([[gorizontals, verticals],
    ...
    def second_karno_figure(neighbor_indexes, col_ind, row_ind, founded):
        ...
        founded.extend([[row_ind, col_ind],
                        [row_ind + 1, col_ind],
                        [row_ind, 4],
                        [row_ind + 1, 4]])
    ...
    def two_line(self, res, neighbor_indexes, founded):
        for index, element in enumerate(res):
            ...
            for col_ind, col_value in enumerate(element):
                if col_ind == 0:
                    continue
                if index != 2 and col_value == self.CONSTANT_ONE and res[index + 1][col_ind] == self.CONSTANT_ONE \
                        and ([index, col_ind] not in founded or [index + 1, col_ind] not in founded):
                    neighbor_indexes.append([index, index + 1, col_ind, col_ind])
                    founded.extend([[index, col_ind], [index + 1, col_ind]])
                elif col_ind != 4 and col_value == self.CONSTANT_ONE and res[index][
                    col_ind + 1] == self.CONSTANT_ONE \
                        and ([index, col_ind] not in founded or [index, col_ind + 1] not in founded):
                    neighbor_indexes.append([index, index, col_ind, col_ind + 1])
                    founded.extend([[index, col_ind], [index, col_ind + 1]])
                elif col_ind == 1 and col_value == self.CONSTANT_ONE and element[-1] == self.CONSTANT_ONE \
                        and ([index, 4] not in founded or [index, col_ind] not in founded):
                    neighbor_indexes.append([index, index, col_ind, -1])
                    founded.extend([[index, col_ind], [index, 4]])
        return neighbor_indexes, founded
    ...
    def get_two(self, res, element, flag):
        ...
        if flag:
            value = self.actions.join(
                [f'!{x}' if x.find('!') == -1 else x.replace('!', '') for x in
                 value.split(self.actions)]
            )
        return value
